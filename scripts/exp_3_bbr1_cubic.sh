...
# output files
declare resultsfile=/home/client/logs/exp_3_${cca}_${loss}_results
...
for ((i=1;i<=${num_runs};i++)) do	
	sudo iperf3 -J --logfile ${resultsfile}_${i}_flow1 -c SERVER_IP -b ${bw} -t ${iperf_time} -i 1 -p ${iperf_port} -C bbr1 --get-server-output &
	sudo iperf3 -J --logfile ${resultsfile}_${i}_flow2 -c SERVER_IP -b ${bw} -t ${iperf_time} -i 1 -p $((${iperf_port}+2)) -C cubic --get-server-output &
	wait
done
